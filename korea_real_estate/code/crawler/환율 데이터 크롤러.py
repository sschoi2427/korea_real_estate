# -*- coding: utf-8 -*-
"""
Created on Tue Mar 10 16:17:57 2020
...
def finance_crawler(first , last):
    ...
    for i in range (first, last+1):
        base_url = "https://finance.naver.com/marketindex/exchangeDailyQuote.nhn?marketindexCd=FX_USDKRW&page={}".format(i) #달러 환율 데이터
        df = df.append(pd.read_html(base_url, header=0)[0], ignore_index=True).dropna()
        
        print(i, '페이지 완료')
    
    df.columns = ["날짜","매매기준율","전일대비","현찰살때","현찰팔때","송금보낼때","송금받을때","T/C살때","외화수표팔때"]
    df = df[::-1] #역순정렬
    print(df[:3])
    
    df.to_csv(RESULT_PATH + 'exchange_data.csv', encoding='euc-kr', index=False)
